...
var checkIfAllowedCafe = function(req, res, next){
...
var checkIfAllowedVisitorsLounge = function(req, res, next){
  User.findById(req.session._id, function(err, user){
...
var checkIfAllowedLobby = function(req, res, next){
  User.findById(req.session._id, function(err, user){
    if ( user.role === "guard" || user.role === "warden" || user.role === "visitor") {
        //console.log("user is logged in. proceeding to next route handler")
        next()
...
var checkIfAllowedWardenOffice = function(req, res, next){
  User.findById(req.session._id, function(err, user){
    if ( user.role === "warden" ) {
        //console.log("user is logged in. proceeding to next route handler")
        next()
...
var checkIfAllowedMalloryCell = function(req, res, next){
  User.findById(req.session._id, function(err, user){
    if ( user.role === "prisoner" && user.username === "mallory") {
        //console.log("user is logged in. proceeding to next route handler")
        next()
    }
    else {
...
var checkIfAllowedEveCell = function(req, res, next){
  User.findById(req.session._id, function(err, user){
    if ( user.role === "prisoner" && user.username === "eve") {
        //console.log("user is logged in. proceeding to next route handler")
        next()
    }
    else {
...
app.get('/cafeteria', checkIfAllowedCafe, function(req, res){
...
app.get('/lobby', checkIfAllowedLobby, function(req, res, next){
    res.sendFile('./html/lobby.html', {root:'./public'});
});
app.get('/visitors-lounge', checkIfAllowedVisitorsLounge, function(req, res, next){
    res.sendFile('./html/visitors-lounge.html', {root:'./public'});
});

app.get('/wardens-office', checkIfAllowedWardenOffice, function(req, res, next){
    res.sendFile('./html/wardens-office.html', {root:'./public'});
});
app.get('/cell-e', checkIfAllowedEveCell, function(req, res, next){
    res.sendFile('./html/cell-e.html', {root:'./public'});
});
app.get('/cell-m', checkIfAllowedMalloryCell, function(req, res, next){
